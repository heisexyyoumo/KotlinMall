...
/*
    订单列表数据适配
 */
class OrderAdapter(context: Context) : BaseRecyclerViewAdapter<Order,
        OrderAdapter.ViewHolder>(context) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (model.orderGoodsList.size == 1){//单个商品
            ...
            for (orderGoods in model.orderGoodsList){//动态添加商品视图
                ...
                val lp = ViewGroup.MarginLayoutParams(mContext
                        .dip(60.0f),mContext.dip(60.0f))
        ...
        holder.itemView.mOrderInfoTv.text = "合计${mTotalCount}件商品，" +
                "总价${YuanFenConverter.changeF2YWithUnit(model.totalPrice)}"


        when(model.orderStatus){//根据订单状态设置颜色、文案及对应操作按钮
            OrderStatus.ORDER_WAIT_PAY -> {
                holder.itemView.mOrderStatusNameTv.text = "待支付"
                holder.itemView.mOrderStatusNameTv
                        .setTextColor(mContext.resources.getColor(R.color.common_red))
                setOptVisible(false,true,true,holder)
            }
            OrderStatus.ORDER_WAIT_CONFIRM -> {
                holder.itemView.mOrderStatusNameTv.text = "待收货"
                holder.itemView.mOrderStatusNameTv
                        .setTextColor(mContext.resources.getColor(R.color.common_blue))
            ...
            OrderStatus.ORDER_COMPLETED -> {
                ...
                holder.itemView.mOrderStatusNameTv
                        .setTextColor(mContext.resources.getColor(R.color.common_yellow))
            ...
            OrderStatus.ORDER_CANCELED -> {
                ...
                holder.itemView.mOrderStatusNameTv
                        .setTextColor(mContext.resources.getColor(R.color.common_gray))
                setOptVisible(false,false,false,holder)
            }
        }
